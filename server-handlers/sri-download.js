...
export async function findInvoice(db, paymentId, decoded) {
    ...
    if (decoded?.uid) {
        const ownLicenseRef = db.collection('users').doc(decoded.uid).collection('licencias').doc(paymentId);
        const ownLicenseSnap = await ownLicenseRef.get();
        if (ownLicenseSnap.exists) {
            return { data: ownLicenseSnap.data() || {}, id: ownLicenseSnap.id, ref: ownLicenseRef, ownerUid: decoded.uid };
        }
    }
...
export async function serveSriArtifact(req, res, kind) {
    ...
    try {
        ...
        const manualArtifactsAvailable = ['ARCHIVOS_MANUALES_REGISTRADOS', 'ARCHIVOS_MANUALES_VERIFICADOS'].includes(String(data.sriEstado || '').toUpperCase())
            && ['PENDING_OWNER_VERIFICATION', 'OWNER_VERIFIED'].includes(String(data.sriManualArtifactsStatus || ''));
        if (String(data.sriEstado || '').toUpperCase() !== 'AUTORIZADO' && !manualArtifactsAvailable) {
