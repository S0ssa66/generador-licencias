...
*   **Plantillas y Textos de Contratos:** [[3_Recursos/Codigo_Beatss/config.js]] (Definición de `DEFAULT_TEMPLATES` y parámetros de limits/licensing).
*   **Compilación y Renderizado (Frontend):** [[3_Recursos/Codigo_Beatss/main.js]] (Funciones `compileContract()`, `generatePreview()` y `downloadPDF()`).
*   **Estilos de Impresión y PDF:** [[3_Recursos/Codigo_Beatss/styles.css]] (Clases `.contract-doc`, `.signature-section`, `.digital-seal-container` y reglas `@media print`).
*   **Generador Backend (Python PDF):**
    *   Generador base: [[3_Recursos/Codigo_Beatss/pdf_generator.py]]
    *   Generador licencias: [[3_Recursos/Codigo_Beatss/generar_pdf_licencias.py]]
