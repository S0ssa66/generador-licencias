...
const loadContacts = (...args) => (typeof window !== 'undefined' && window.loadContacts ? window.loadContacts(...args) : undefined);
const safeSetItem = (...args) => (typeof window !== 'undefined' && window.safeSetItem ? window.safeSetItem(...args) : undefined);
const saveAllContacts = (...args) => (typeof window !== 'undefined' && window.saveAllContacts ? window.saveAllContacts(...args) : undefined);
const updateHistoryTable = (...args) => (typeof window !== 'undefined' && window.updateHistoryTable ? window.updateHistoryTable(...args) : undefined);
const loadConsolidatedAccounting = (...args) => (typeof window !== 'undefined' && window.loadConsolidatedAccounting ? window.loadConsolidatedAccounting(...args) : undefined);
const saveHistory = (...args) => (typeof window !== 'undefined' && window.saveHistory ? window.saveHistory(...args) : undefined);
...
const sanitizeImportText = (val, max = 240) => String(val ?? '')
    .replace(/[<>]/g, '')
    .trim()
    .slice(0, max);
    ...
    reader.onload = async function(evt) {
        try {
            ...
            for (let i = 0; i < rows.length; i++) {
                ...
                const invoice = sanitizeImportText(row["Invoice Number"] || lastInvoice, 128);
                const dateRaw = sanitizeImportText(row["Date"] || lastDate, 64);
                const customerName = sanitizeImportText(row["Customer Name"] || lastCustomerName, 160);
                const customerEmail = sanitizeImportText(row["Customer Email"] || lastCustomerEmail, 240);
                ...
                const rawItemName = sanitizeImportText(row["Item Name"] || "", 200);
                if (!rawItemName) continue;

                const cleanedBeat = cleanBeatName(rawItemName);
...
if (typeof window !== 'undefined') {
    ...
    window.sanitizeImportText = sanitizeImportText;
}

export { sanitizeImportText, cleanBeatName, parseCSV };
