...
function initSalesRealtimeListener() {
    ...
    window._salesUnsubscribe = onSnapshot(q, (snapshot) => {
        ...
        snapshot.docChanges().forEach(change => {
            ...
            if (data.type !== 'beat_purchase' && data.type !== 'exclusive_offer' && data.type !== 'license_upgrade') return;
        ...
        snapshot.forEach(d => {
            const data = d.data();
            if (data.type === 'beat_purchase' || data.type === 'exclusive_offer' || data.type === 'license_upgrade') {
        ...
        if (!_salesFirstLoad && newDocs.length > 0) {
            newDocs.forEach(pay => {
                ...
                if (window.Notification?.permission === 'granted') {
                    new window.Notification(`${icon} BEATSS – ${title}`, {
...
function renderSalesTable() {
    ...
    const tablePanel = document.getElementById('sales-table-panel');
    ...
    tbody.innerHTML = window.storePayments.map(pay => {
        const safeId = encodeURIComponent(String(pay.id || ''));
        const dateStr = sanitizeHtml(pay.timestamp ? pay.timestamp.split('T')[0] : 'N/A');
        const isOffer = pay.type === 'exclusive_offer';
        const methodLabels = {
            stripe: 'Stripe',
            deuna: 'Deuna!',
        ...
        const methodLabel = isOffer ? '💬 Oferta' : sanitizeHtml(methodLabels[pay.method] || 'Otro');
        ...
        if (pay.status === 'pending') {
            ...
            if (isOffer) {
                actionButtons = `
                    <button class="btn btn-primary" onclick="window.acceptExclusiveOffer(decodeURIComponent('${safeId}'))" style="padding: 4px 8px; font-size: 11px; height: 26px;">✓ Aceptar</button>
                    <button class="btn" onclick="window.rejectBeatSale(decodeURIComponent('${safeId}'))" style="padding: 4px 8px; font-size: 11px; height: 26px; background: rgba(239, 68, 68, 0.1); border: 1px solid rgba(239, 68, 68, 0.2); color: #ef4444; margin-left: 4px;">✕ Rechazar</button>
                `;
            } else {
                actionButtons = `
                    <button class="btn btn-primary" onclick="window.approveBeatSale(decodeURIComponent('${safeId}'))" style="padding: 4px 8px; font-size: 11px; height: 26px;">Aprobar</button>
                    <button class="btn" onclick="window.rejectBeatSale(decodeURIComponent('${safeId}'))" style="padding: 4px 8px; font-size: 11px; height: 26px; background: rgba(239, 68, 68, 0.1); border: 1px solid rgba(239, 68, 68, 0.2); color: #ef4444; margin-left: 4px;">Rechazar</button>
                `;
            }
        ...
        const receiptLink = (pay.receiptUrl && isSafeArtworkUrl(pay.receiptUrl))
            ? `<a href="#" onclick="event.stopPropagation(); window.viewReceiptImage(decodeURIComponent('${safeId}'))" style="color: var(--accent); font-weight: 600; text-decoration: underline;">Ver Captura</a>` 
            : `<span style="color: #8a91a6;">${isOffer ? 'No aplica' : 'Sin comprobante'}</span>`;

        // Mostrar precio propuesto en la oferta vs precio original
        const priceDisplay = isOffer
            ? `<div style="font-weight: 700; color: #f59e0b;">$${parseFloat(pay.price || 0).toFixed(2)}</div><div style="font-size: 10px; color: #8a91a6;">Precio orig: $${parseFloat(pay.originalPrice || pay.price || 0).toFixed(2)}</div>`
            : `<div style="font-weight: 700; color: #fff;">$${parseFloat(pay.price || 0).toFixed(2)}</div>`;

        // Mensaje de la oferta si existe
        const offerMessageRow = isOffer && pay.offerMessage
            ? `<tr><td colspan="8" style="background: rgba(245, 158, 11, 0.05); padding: 8px 12px; font-size: 12px; color: #8a91a6; font-style: italic; border-bottom: 1px solid rgba(255,255,255,0.04);">💬 Mensaje: "${sanitizeHtml(pay.offerMessage)}"</td></tr>`
            : '';
        ...
        const safeReference = sanitizeHtml(pay.reference || 'N/A');
        const safeBeatName = sanitizeHtml(pay.beatName || 'Beat');
        const safeLicenseType = sanitizeHtml(pay.licenseType || '');
        const safeBuyerName = sanitizeHtml(pay.buyerName || 'Anónimo');
        const safeBuyerEmail = sanitizeHtml(pay.buyerEmail || '');

        return `
            ${offerMessageRow}
            <tr onclick="if(!event.target.closest('button') && !event.target.closest('a')) window.openSaleDetailsModal(decodeURIComponent('${safeId}'))" style="cursor: pointer; ${isOffer ? 'border-left: 3px solid #f59e0b;' : (isNew ? 'border-left: 3px solid #ef4444;' : '')}">
                <td data-label="Fecha / ID">
                    <div style="font-weight: 600;">${dateStr}${newBadge}</div>
                    <div style="font-size: 10px; color: #8a91a6;">ID: ${safeReference}</div>
                </td>
                <td data-label="Beat / Licencia">
                    <div style="font-weight: 600; color: #fff;">${safeBeatName}</div>
                    <div style="font-size: 11px; text-transform: uppercase; color: var(--accent);">${safeLicenseType}${isOffer ? ' 🏷️' : ''}</div>
                </td>
                <td data-label="Comprador">
                    <div>${safeBuyerName}</div>
                    <div style="font-size: 11px; color: #8a91a6;">${safeBuyerEmail}</div>
                </td>
                <td data-label="Monto">${priceDisplay}</td>
                <td data-label="Método">${methodLabel}</td>
                <td data-label="Comprobante">${receiptLink}</td>
                <td data-label="Estado">${statusBadge}</td>
                <td class="actions-cell" data-label="Acciones" style="text-align: right; white-space: nowrap;">${actionButtons}</td>
            </tr>
        `;
    }).join('');
...
async function openSaleDetailsModal(paymentId) {
    ...
    // Ocultar sección de comprobante al cargar
    const loadingReceiptContainer = document.getElementById('admin-sale-receipt-container');
    if (loadingReceiptContainer) loadingReceiptContainer.style.display = 'none';
    ...
    try {
        ...
        // Poblar beat details
        if (beat.artwork && isSafeArtworkUrl(beat.artwork)) {
        ...
        // Poblar comprobante de pago
        const receiptContainer = document.getElementById('admin-sale-receipt-container');
        const receiptImg = document.getElementById('admin-sale-receipt-img');
        if (receiptContainer && receiptImg) {
            if (payment.receiptUrl && isSafeArtworkUrl(payment.receiptUrl)) {
                receiptImg.src = payment.receiptUrl;
                receiptImg.dataset.paymentId = payment.id;
        ...
        if (!downloads || downloads.length === 0) {
            ...
            historyList.innerHTML = downloads.map(log => {
                const timestampFormatted = sanitizeHtml(log.timestamp ? log.timestamp.split('.')[0] + 'Z' : 'N/A');
                const fileTypeLabel = sanitizeHtml({
                    mp3: 'MP3',
                    wav: 'WAV',
                    stems: 'Stems',
                    license: 'Licencia PDF'
                }[log.fileType] || log.fileType || '');
                const safeIp = sanitizeHtml(log.ip || '');
                
                return `<div class="flex justify-between items-center py-1 border-b border-white/[0.02]">
                    <span>${timestampFormatted} [${safeIp}]</span>
...
async function approveBeatSale(paymentId) {
    ...
    try {
        ...
        const deliverySucceeded = await sendEmailDelivery(paymentId);
        if (!deliverySucceeded) {
            throw new Error('La licencia no se envió. La venta permanecerá pendiente para poder reintentar.');
        }
...
async function acceptExclusiveOffer(paymentId) {
    ...
    try {
        const exclusiveEffectiveAt = new Date().toISOString();
        ...
        document.getElementById('buyer-country').value = payment.buyerCountry || "";
        document.getElementById('ref-code').value = payment.reference;
        document.getElementById('effective-date').value = exclusiveEffectiveAt.slice(0, 10);
        document.getElementById('audio-link-mp3').value = beatData.mp3 || "";
        document.getElementById('audio-link-wav').value = beatData.wav || "";
        document.getElementById('audio-link-stems').value = beatData.stems || "";
        ...
        const deliverySucceeded = await sendEmailDelivery(paymentId);
        if (!deliverySucceeded) {
            throw new Error('La licencia no se envió. La oferta permanecerá pendiente para poder reintentar.');
        }

        // 4. Cerrar la venta pública y registrar la fecha de corte. Esa fecha
        // protege las licencias no exclusivas emitidas antes de la exclusiva y
        // permite que sólo esos titulares puedan ampliar hasta Ilimitada.
        const payDocRef = doc(db, "payments", paymentId);
        const exclusiveRecord = {
            status: 'approved',
            contractEffectiveAt: exclusiveEffectiveAt,
            exclusiveAcceptedAt: exclusiveEffectiveAt,
            updatedAt: exclusiveEffectiveAt
        };
        await updateDoc(payDocRef, exclusiveRecord);
        if (beatSnap.exists()) {
            await updateDoc(beatDocRef, {
                sold: true,
                isSold: true,
                exclusiveSoldAt: exclusiveEffectiveAt,
                exclusiveEffectiveAt,
                exclusivePaymentId: paymentId,
                exclusiveLicense: {
                    paymentId,
                    effectiveAt: exclusiveEffectiveAt
                },
                updatedAt: exclusiveEffectiveAt
            });
        }
...
function printReceipt() {
    ...
    if (!receiptBody) {
        window.showToast?.('No hay un recibo abierto para imprimir.', true);
        return;
    }

    // Abrir la vista de impresión durante el gesto del botón evita bloqueos de
    // popup y mantiene el resto del Studio fuera del documento impreso.
    const printWindow = window.open('', '_blank', 'width=780,height=920');
    if (!printWindow) {
        window.showToast?.('El navegador bloqueó la ventana de impresión. Permite ventanas emergentes para BEATSS.', true);
        return;
    }

    printWindow.opener = null;
    printWindow.document.write(`<!doctype html>
        <html lang="es"><head><meta charset="utf-8"><title>Recibo BEATSS</title>
        <style>
            @page { size: auto; margin: 18mm; }
            * { box-sizing: border-box; }
            body { margin: 0; color: #10151f; background: #fff; font: 14px/1.55 Inter, Arial, sans-serif; }
            h1, h2, h3, h4 { color: #10151f; }
            table { width: 100%; border-collapse: collapse; }
            th, td { padding: 8px; border-bottom: 1px solid #dfe2e8; text-align: left; }
            img { max-width: 100%; height: auto; }
            .no-print { display: none !important; }
        </style></head><body>${receiptBody.innerHTML}</body></html>`);
...
function viewReceiptImage(paymentId) {
    const pay = window.storePayments.find(p => p.id === paymentId);
    if (!pay || !pay.receiptUrl || !isSafeArtworkUrl(pay.receiptUrl)) return;
    
    let lightbox = document.getElementById('receipt-lightbox');
    if (!lightbox) {
        lightbox = document.createElement('div');
        lightbox.id = 'receipt-lightbox';
        lightbox.style.position = 'fixed';
        lightbox.style.top = '0';
        lightbox.style.left = '0';
        lightbox.style.width = '100vw';
        lightbox.style.height = '100vh';
        lightbox.style.backgroundColor = 'rgba(10,12,18,0.95)';
        ...
        lightbox.innerHTML = `
            <div style="position: relative; max-width: 90%; max-height: 90%; display: flex; align-items: center; justify-content: center;">
                <img id="receipt-lightbox-img" src="" style="max-width: 100%; max-height: 85vh; border-radius: 12px; border: 1px solid rgba(255,255,255,0.1); box-shadow: 0 20px 40px rgba(0,0,0,0.5); object-fit: contain;" />
                <button style="position: absolute; top: -40px; right: 0; background: none; border: none; color: white; font-size: 24px; cursor: pointer;">&times;</button>
            </div>
        `;
    ...
    document.getElementById('receipt-lightbox-img').src = pay.receiptUrl;
    lightbox.style.display = 'flex';
}
